// github_sync.js
// ═══════════════════════════════════════════════════════════════════════
//  يزامن الملفات المحلية مع مستودع GitHub قبل تشغيل البوت. يستدعيها
//  index.js كأول خطوة عند الإقلاع. يسحب فقط الملفات التي تغيرت (وليس
//  المستودع كاملاً) بالاعتماد على آخر SHA محفوظ محلياً، ويحذف محلياً أي
...
const IGNORE_PATTERNS = [
  'node_modules/',
  '.git/',
  'github_sync.js',    // لا يستبدل نفسه أثناء التنفيذ
  '.sync_state.json',
  'package-lock.json',
];
...
async function downloadFile(sha, repoPath) {
...
async function fullSync(sha) {
  ...
  for (const file of files) {
    try {
      await downloadFile(sha, file.path);
    } catch (e) {
...
async function incrementalSync(oldSha, newSha) {
  ...
  for (const file of changedFiles) {
    ...
    try {
      await downloadFile(newSha, file.filename);
...
async function syncFromGitHub() {
  ...
  if (!state.lastSha) {
    await fullSync(latestSha);
  } else {
    await incrementalSync(state.lastSha, latestSha);
  }
